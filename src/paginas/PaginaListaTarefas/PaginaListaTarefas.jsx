...
const PaginaListaTarefas = () => {
  ...
  const removerDaLista = (index) => {
    if (
      confirm(
        `Tem certeza que deseja excluir a tarefa: ${tarefas[index].descricao}`
      )
    ) {
      if (tarefas[index].feita) {
        ...
        alert("A tarefa precisa estar concluida para ser excluída.");
      }
    }
  };
          ...
          return (
            <div key={index} className="pagina-lista-tarefas_item">
              <input
                type="checkbox"
                checked={item.feita}
                onChange={() => marcarComoFeito(index)}
              />
              <span
                style={{ textDecoration: item.feita ? "line-through" : "none" }}
              >
                {item.descricao}
              </span>
